...
public class ArgParser {
        ...
        for (int i = 0; i < paramsSchemaList.size(); i++) {
            ...
            if (schema.isEmpty()) {
                throw new IllegalArgumentException(AppSettings.global.translations.get(TranslationKey.invalid_argument) + i
                        + ": " + argName + ", "
                        + AppSettings.global.translations.get(TranslationKey.expected_one_of) + ": " + paramsSchemaList.get(i));
            }
            if (argValues.containsKey(argName)) {
                throw new IllegalArgumentException( AppSettings.global.translations.get(TranslationKey.argument_already_provided).replace("{argName}", argName));
            }
            Object value;
            if(schema.get().getParamType() == ParamType.NUMBER){
                try {
                    ...
                    throw new IllegalArgumentException(AppSettings.global.translations.get(TranslationKey.expected_positive_number).replace("{argName}", argName));
                }
            }
            else{
                value = params[i * 2 + 1];
    ...
    public Param getArgValue(String argName) {
        if (!argValues.containsKey(argName)) {
            throw new IllegalArgumentException(AppSettings.global.translations.get(TranslationKey.argument_not_found).replace("{argName}", argName));
        }
